...
**重要**: ��ーザーが明���的に承認するまで `update_profile` を呼び出してはならない。

**部分適用の場合**: ユーザーが「1と3を適用」のように指定した場合、適用する変更内容を再度リストで表示し、「以��の変更を適用してよろしいですか？」と最終確認を行ってから `update_profile` を呼び出す。ミスパースによる意図しない更新を防ぐため。

ユーザーが拒否した場合は「承知しま��た。プロフィールは��更しません。」と伝えて終了する。
...
2. `update_profile` — プロ��ィール更新（ユーザー承認時のみ。レス��ンスに更新後プロフィールが含まれるため、確認用の再取���は不要）
