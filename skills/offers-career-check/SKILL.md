---
name: offers-career-check
description: "スキルセットベースの年収相場チェック。現在のスキルで市場年収の下位帯/中央値/上位帯を算出する。トリガー: /offers-career-check, 年収相場, career check"
---

# offers-career-check

ユーザーのスキルセットに基づいて、求人市場の年収相場を算出し、現在年収との比較と年収アップにつながるスキル提案を行う。

## 実行フロー

### Step 1: プロフィール取得

MCP ツール `get_profile` を呼び出し、以下を取得する。

- `skills` 配列 — ユーザーが登録しているスキル一覧（各要素に `id`, `name` を含む）
- `career_intention.current_annual_pay` — 現在の年収（万円単位、未設定の場合は null）

**スキルが 0 件の場合**: 以下のメッセージを表示して終了する。

```
スキルが登録されていないため、年収相場を算出できません。
まず /offers-profile-review でプロフィールを整備してください。
```

### Step 2: 求人検索

プロフィールのスキル ID を使い、MCP ツール `search_jobs` を呼び出す。

```
search_jobs({ skillIds: [ユーザーのスキルID一覧], limit: 50 })
```

- `limit: 50` で十分な統計母数を確保する
- スキル数が多い場合でも、全スキル ID をまとめて 1 回で検索する

### Step 3: 年収統計の算出

#### 3.1 年収の年間換算

MCP レスポンスに含まれる `annualized_pay_min` と `annualized_pay_max` をそのまま使用する。
これらは以下のルールで事前計算済みの値である。

| pay_type | 換算ルール |
|----------|-----------|
| ANNUAL / ANNUALLY | そのまま（万円） |
| MONTHLY | value x 12（万円） |
| HOURLY | Math.round(value x 8 x 20 x 12 / 10000)（概算、万円） |
| UNSET / null | 統計から除外 |

`annualized_pay_min` と `annualized_pay_max` のいずれかが null または 0 の求人は統計から除外する。

#### 3.2 中央値の算出

各求人の代表年収を算出する。

```
job_median = (annualized_pay_min + annualized_pay_max) / 2
```

全求人の `job_median` を昇順にソートし、以下の 3 値を求める。

- **下位帯（25 パーセンタイル）**: ソート済み配列の 25% 位置の値
- **中央値（50 パーセンタイル）**: ソート済み配列の 50% 位置の値
- **上位帯（75 パーセンタイル）**: ソート済み配列の 75% 位置の値

### Step 4: 現在年収との比較

`current_annual_pay` が取得できた場合、中央値との差分を算出し、市場におけるポジションを判定する。

| 条件 | 判定 |
|------|------|
| 現在年収 >= 上位帯 | 市場上位 — 現在のスキルセットでは高水準 |
| 現在年収 >= 中央値 | 市場平均以上 — 競争力のある水準 |
| 現在年収 < 中央値 | 市場平均未満 — 年収アップの余地あり |

### Step 5: 年収アップにつながるスキル提案

Step 2 で取得した求人のうち、`job_median` が上位帯以上の高年収求人を抽出する。
それらの求人に求められているスキルのうち、ユーザーのプロフィールに含まれていないスキルを集計し、出現頻度の高い順に最大 5 件を提案する。

## MCP 呼び出し制約

合計 3 回以内に収める。

1. `get_profile` — 1 回
2. `search_jobs` — 1 回
3. （必要に応じて）`search_skills` — 1 回（提案スキルの詳細取得など）

## 出力フォーマット

以下の形式で結果を表示する。

```
## 年収相場レポート

あなたのスキルセット（React, TypeScript, Node.js 等）に基づく求人市場の年収相場です。

### 市場年収レンジ

| 区分 | 年収（万円） |
|------|-------------|
| 下位帯（25%ile） | 450 |
| 中央値（50%ile） | 600 |
| 上位帯（75%ile） | 800 |

対象求人数: 42 件

### 現在年収との比較

現在年収: 550 万円
市場中央値との差: -50 万円（市場平均未満）

市場平均に対してやや低い水準です。以下のスキル習得で上位帯を狙える可能性があります。

### 年収アップにつながるスキル

上位帯（800万円以上）の求人で多く求められているスキルのうち、
あなたのプロフィールに未登録のものです。

1. **Kubernetes** — 高年収求人の 60% で要求
2. **AWS** — 高年収求人の 45% で要求
3. **GraphQL** — 高年収求人の 35% で要求

---
※ 本レポートは Offers 掲載求人の給与情報に基づく参考値です。
※ プロフィール情報はレポート生成のみに使用し、外部には共有しません。
```

## エラーハンドリング

- **MCP API エラー**: 「年収相場の取得に失敗しました。しばらく時間をおいて再度お試しください。」と表示する
- **有効求人が 5 件未満**: 統計結果を表示しつつ、以下の注記を付与する
  ```
  ※ 対象求人が少ないため（N 件）、統計の信頼性が低い可能性があります。
  ```
- **有効求人が 0 件**: 「お持ちのスキルセットに合致する給与情報付きの求人が見つかりませんでした。」と表示する

## 注意事項

- レスポンスはすべて日本語で記述する
- 金額は万円単位で表示し、小数点以下は四捨五入する
- プロフィール情報の取り扱いに関するプライバシー注記を必ず末尾に含める
- 処理効率を重視し、不要な MCP 呼び出しを行わない
